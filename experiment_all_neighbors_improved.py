...
# + Original mode + MEDOID mode (Gower + KMedoids)
...
# ---------------------------------------------------------
# Gower distance helpers (RAW space, for KMedoids)
# ---------------------------------------------------------
def gower_distance_to_all_raw(X, x, feature_names, symptom_cols,
                              TEST_BOUNDS, symptom_range=(0, 10)):
    """
    X: array-like (n, d) of RAW neighbors
    x: array-like (d,) point in the same RAW feature space
...
def compute_gower_distance_matrix_raw(X, feature_names, symptom_cols,
                                      TEST_BOUNDS, symptom_range=(0, 10)):
    """
...
output_path = "anchor_experiment_true_label_exclusion_ALL_neighbors_MEDOID_with_applicability_5_100.txt"

with open(output_path, "w") as f:
    f.write("EXPERIMENT: exclude TRUE label, using ALL neighbors\n")
    f.write("Original mode + MEDOID mode (Gower + KMedoids)\n")
    ...
    for run_id, new_patient_idx in enumerate(selected_test_indices, 1):
        ...
        f.write(f"Neighbors excluding TRUE label: {len(neighbors_excluding_target)}\n\n")

        # ---------------------------------------------------------
        # 5) MEDOIDS with RAW Gower (cluster neighbors_excluding_target_raw)
        # ---------------------------------------------------------
        ...
        # Precompute RAW Gower distance matrix among neighbors
        ...
        # Number of clusters: use number of unique labels (capped by n_pts)
        n_pts = len(X_cluster_raw_df)
        n_labels_here = len(np.unique(neighbors_labels))
        k = min(max(1, n_labels_here), n_pts)

        f.write(f"KMedoids: n_pts={n_pts}, k={k}, D_shape={D.shape}\n")

        kmedoids = KMedoids(
            n_clusters=k,
            metric="precomputed",
            init="k-medoids++",  # let KMedoids choose good initial medoids
            max_iter=300,
            random_state=0
        )
        kmedoids.fit(D)
        ...
        f.write(f"Medoid indices (raw space): {medoids_idx}\n\n")
        ...
        # Bin medoids so they match explainer feature space
        ...
        # Force generic symptoms = patient values
        ...
        for c_id, centroid_instance in enumerate(final_medoids_binned, 1):
            exp_c, valid_anchors_c = explainer.explain_instance(
                centroid_instance,
                xgb_cl,
                mode="conformal",
            ...
            f.write(f"\n--- Medoid {c_id}/{len(final_medoids_binned)} ---\n")
            main_names_c = exp_c.names()
            f.write("Main anchor: %s\n" % (' AND '.join(main_names_c)))
            f.write("Precision: %.3f\n" % exp_c.precision())
            f.write("Coverage: %.5f\n" % exp_c.coverage())
            f.write("Cumulative coverage: %.5f\n" % exp_c.cumulative_coverage())
            f.write(f"---> valid anchors found: {len(valid_anchors_c)} {[d['feature'] for d in valid_anchors_c]}\n")

            main_applies = anchor_applies_to_instance(main_names_c, new_patient)
            f.write(f"Does MAIN anchor (medoid {c_id}) apply to patient? {main_applies}\n")

            num_valid_apply = 0
            for va in valid_anchors_c:
                if anchor_applies_to_instance(va['names'], new_patient):
                    num_valid_apply += 1
            f.write(
                f"#valid anchors (medoid {c_id}) applying to patient: "
                f"{num_valid_apply} / {len(valid_anchors_c)}\n"
            )

            if len(valid_anchors_c) > 0:
                num_feats_each_c = [len(va['feature']) for va in valid_anchors_c]
                avg_feats_c = float(np.mean(num_feats_each_c))
                unique_feats_c = set()
                for va in valid_anchors_c:
                    unique_feats_c |= set(va['feature'])
                num_unique_feats_c = len(unique_feats_c)
            else:
                avg_feats_c = 0.0
                num_unique_feats_c = 0

            f.write(f"Valid anchors (medoid {c_id}): {len(valid_anchors_c)}\n")
            f.write(f"Avg #features per valid anchor (medoid {c_id}): {avg_feats_c:.3f}\n")
            f.write(f"#unique features across valid anchors (medoid {c_id}): {num_unique_feats_c}\n")

            sorted_valid_c = sorted(
                valid_anchors_c,
                key=lambda va: va['coverage'][-1],
                reverse=True
            )
            for j, va in enumerate(sorted_valid_c, 1):
                names = " AND ".join(va['names'])
                prec = va['precision'][-1]
                cov = va['coverage'][-1]
                applies = anchor_applies_to_instance(va['names'], new_patient)
            ...
            results.append({
                ...
                'mode': 'medoid',
                'medoid_id': c_id,
                'main_precision': exp_c.precision(),
                'main_coverage': exp_c.coverage(),
                'cumulative_coverage': exp_c.cumulative_coverage(),
                'avg_feats_valid': avg_feats_c,
                'num_unique_feats_valid': num_unique_feats_c,
                'num_valid_anchors': len(valid_anchors_c),
    ...
    # ---------------------------------------------------------
    f.write("\n" + "=" * 100 + "\n")
    f.write("GLOBAL STATISTICS ACROSS ALL (INSTANCE, TRUE_LABEL_EXCLUSION, MODE) TRIPLES\n")
    f.write("=" * 100 + "\n\n")

    f.write(f"Number of instances with no neighbors' prediction sets excluding true label: {counter}\n\n")

    for mode in ['original', 'medoid']:
        ...
        f.write(
            f"  #runs with NO anchors (main nor any valid) applying to patient: "
            f"{count_no_anchor_applies} / {len(mode_results)}\n"
        )
        f.write(
            f"  Fraction of runs with NO anchors applying: "
